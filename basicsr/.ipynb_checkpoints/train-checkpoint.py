...
def main():
    ...
    if resume_state:  # resume training
        ...
        start_epoch = 0
        current_iter = 0
    if opt.get('network_g', {}).get('frozen_layers'):
        frozen_layers = opt['network_g']['frozen_layers']
        if isinstance(frozen_layers, (list, tuple)) and len(frozen_layers) > 0:
            logger.info(f"❄️ 开始冻结以下层: {frozen_layers}")
            for name, param in model.net_g.named_parameters():
                if any(name.startswith(layer_prefix) for layer_prefix in frozen_layers):
                    param.requires_grad = False
                    logger.info(f"  🔒 已冻结: {name}")
            logger.info("✅ 指定层冻结完成")
        else:
            logger.warning("⚠️ network_g.frozen_layers 存在但为空或格式错误")
...
#     # ========================== 核心修改点：冻结指定层 ==========================
#     # 检查配置文件中是否存在 'finetune_settings:frozen_layers' 这个我们自定义的选项
#     if opt.get('finetune_settings', {}).get('frozen_layers') is not None:
#         layers_to_freeze = opt['finetune_settings']['frozen_layers']
#         if layers_to_freeze:
#             logger.info(f"❄️ 正在冻结以下层: {layers_to_freeze}")
#             # 遍历模型(net_g)的所有参数
#             for name, param in model.net_g.named_parameters():
#                 # 检查参数名是否以任何一个要冻结的层名开头
#                 if any(name.startswith(layer_name) for layer_name in layers_to_freeze):
#                     param.requires_grad = False # 将参数的梯度属性设为False，使其不可训练
#                     logger.info(f"  - 已冻结: {name}")
#     # =======================================================================
